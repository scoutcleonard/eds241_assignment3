...
\noindent The average treatment effect of maternal smoking on birth weight when all other covariants are held equal is -228.07 grams (on average, infants of mothers who use tobacco weigh 236.46 grams less than the infants of mothers who do not), with a standard error of 4.28.
...
smoking_df <- smoking_df %>% 
  ...
  mutate(meduc_indicator = case_when(meduc >= 16 ~ 1,
                                     meduc < 16 ~ 0)) %>% 
  mutate(g = paste0(as.factor(mage_indicator), 
